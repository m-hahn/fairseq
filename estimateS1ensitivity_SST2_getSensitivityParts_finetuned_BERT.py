...
with open(f"/u/scr/mhahn/PRETRAINED/GLUE/glue_data/SST-2/dev_alternatives_predictions_finetuned_BERT.tsv", "r") as inFile:
  for line in inFile:
     if len(line) < 5:
       continue
...
BERT_alternatives = defaultdict(list)
for group in ["", "_d", "_e"]:
 with open(f"/u/scr/mhahn/PRETRAINED/GLUE/glue_data/SST-2/dev_alternatives_BERT_finetuned{group}.tsv", "r") as inFile:
  for line in inFile:
     line = line.strip().split("\t")
     if len(line) < 3:
           print("ERROR", line)
           continue
     BERT_alternatives[(line[0].strip(), line[1].strip())].append(line[2])
 ...
 #  print(original+"#")
  ...
  # print(entry)
   ...
   for variant in alternative[3:]:
      ...
      if ((subset, tokenized2) not in BERT_alternatives):
         print("WEIRD", (subset, tokenized2))
      if (subset, tokenized2) in processed:
        continue
      for alternative in BERT_alternatives[(subset, tokenized2)]:
         #print(alternative)
