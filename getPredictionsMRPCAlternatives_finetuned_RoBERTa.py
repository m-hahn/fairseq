...
roberta.eval()
evaluatedSoFar = set()
with open('/u/scr/mhahn/PRETRAINED/GLUE/glue_data/MRPC/dev_alternatives_predictions_finetuned_RoBERTa.tsv', "w") as outFile:
 for group in ["", "_d", "_e"]:
  try:
   with open(f'/u/scr/mhahn/PRETRAINED/GLUE/glue_data/MRPC/dev_alternatives_RoBERTa_finetuned{group}.tsv', 'r') as fin:
    while True:
        line = next(fin).strip()
        try:
