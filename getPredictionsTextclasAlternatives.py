...
roberta.eval()
evaluatedSoFar = set()
with open(f'/u/scr/mhahn/PRETRAINED/textclas/{model}_alternatives_predictions_fairseq.tsv', "w") as outFile:
 for group in ["", "_d"]:
  with open(f'/u/scr/mhahn/PRETRAINED/textclas/{model}_alternatives{group}.txt') as fin:
   try:
    while True:
        line = next(fin).strip()
        if line == "#####":
