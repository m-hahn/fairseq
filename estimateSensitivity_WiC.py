...
#with open(f"/u/scr/mhahn/PRETRAINED/GLUE/glue_data/WiC/dev_alternatives_predictions_fairseq.tsv", "r") as inFile:
#  for line in inFile:
#     if len(line) < 5:
#       continue
...
POSITIVE_RATIO_DATASET = 0.6716428344455037 #0.16 #0.3652573

#print(list(alternatives_predictions_binary.items())[:10])

with open(f"/u/scr/mhahn/PRETRAINED/SuperGLUE/WiC/dev_alternatives_predictions_fairseq.tsv", "r") as inFile:
...
with open(f"/u/scr/mhahn/sensitivity/sensitivities/sensitivities_{__file__}", "w") as outFile:
 ...
 for alternative in alternatives:
   ...
   for variant in alternative[2:]:
      ...
      try:
         subset, sentence1, sentence2, continuous, binary = variant.strip().split("\t")
      except ValueError:
      ...
      cont = math.exp(float(continuous))
      assert cont <= 1
      assert cont >= -1
      cont = 2*cont - 1
      alternatives_predictions_binary[sentence.strip()] = {"False" : "0", "True" : "1"}[binary.strip()]
      alternatives_predictions_float[sentence.strip()] = float(cont)

      averageLabel[0]+=1
      averageLabel[1]+=(float(cont))
      averageLabel[2]+=((float(cont)))**2
      averageLabel[3]+=(float(alternatives_predictions_binary[sentence.strip()]))
