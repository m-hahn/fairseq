...
assert task in ["mr", "cr", "mpqa", "subj"]
...
with open(f'/u/scr/mhahn/PRETRAINED/textclas/{task}_alternatives_predictions_PMLM_1billion_fairseq.tsv', "r") as inFile:
  for line in inFile:
     if len(line) < 5:
       continue
...
for group in ["", "_d", "_e", "_f"]:                                                                                                                                                                        
 try:
  with open(f"/u/scr/mhahn/PRETRAINED/textclas/{task}_alternatives_finetuned{group}.txt", "r") as inFile:
...
for group in [""]: # , "_d", "_e"
 with open(f'/u/scr/mhahn/PRETRAINED/textclas/{task}_alternatives_PMLM_1billion_raw.tsv', "r") as inFile:
  for line in inFile:
     line = line.strip().split("\t")
     if len(line) < 3:
           print("ERROR", line)
...
with open(f"/u/scr/mhahn/sensitivity/sensitivities/s1ensitivities_{__file__}_{task}", "w") as outFile:
