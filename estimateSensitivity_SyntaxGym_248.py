...
predictions = {}
sentencesInOrder = []
with open(f"/u/scr/mhahn/PRETRAINED/SyntaxGym/syntaxgym_248_alternatives_raw_grnn.tsv", "r") as inFile:
  header = next(inFile)
  assert header == "sentence_id\ttoken_id\ttoken\tsurprisal\n"
  data = inFile.read().split("<eos>")
  for sentence in data:
    ...
    if len(sentence) < 3 or len(sentence[-3]) < 3:
      ...
      continue
    assert sentence[-3][2] in ["himself", "themselves"], sentence[-3][2]
    reflexive = sentence[-3][2]
